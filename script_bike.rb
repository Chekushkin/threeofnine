...
uri   = "https://999.md/ru/list/transport/bicycles?view_type=short"
...
def convert(string)
  ...
  month_int = "%02d" % (get_index(date) + 1)
...
html.css("table.ads-list-table tr").select { |tr| tr.css(".ads-list-table-price").present? && !tr.css(".ads-list-table-price").text.blank? }.each_with_index do |item, index|
  ...
  date = Date.strptime(convert(html.css("dd").detect { |dd| dd.text.match(/^\d{1,2}\s/) }.text.split(",").first.gsub(".", "")), "%d %m %Y")
  ...
  unless html.css("div.adPage__content__features__col.grid_7.suffix_1 li").blank?
    ...
    html.css("div.adPage__content__features__col.grid_7.suffix_1 li").each { |li| data[index][:additional] << parse_description(li.text) }
  end
...
# */59 * * * * /Users/eduardmacarov/.rvm/wrappers/ruby-2.2.2/ruby /Users/eduardmacarov/Desktop/999_notifier/script.rb